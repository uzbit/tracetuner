...
/* Function prototypes:  
   --------------------
*/
int process(char* filename, BtkMessage* message);
...
// Store command line arguments as integer variables
   ...
   if (filename > 0)
   {
      r = F_Open(filename, &p, &fileSize);
          if (r == ERROR) {
	    (void)sprintf(message->text, "File not found:%s", filename);
	    return r;
	  }

      r = read_nums(&num_called_bases, &num_datapoints);
          if (r == ERROR) {
	    (void)sprintf(message->text, "Error in read_nums.");
	    return r;
	  }
    ...
    )
{
     ...
     */
    if (path != NULL) {
        /* If a path has been specified, use it plus the filename
	 * (without path) plus ".phd.1". */
	...
	/* Look in the same directory as the sample file. */
	(void)sprintf(phd_file_name, "%s.phd.1", file_name);
    }
